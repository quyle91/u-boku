...
abstract class Base
{
    ...
    function encrypt($plaintext)
    {
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            \DUP_PRO_LOG::trace('ENGINE_OPENSSL engine used for SFTP encryption');
            if ($this->changed) {
                $this->_clearBuffers();
                $this->changed = false;
            }
        ...
        if ($this->engine === self::ENGINE_MCRYPT) {
            \DUP_PRO_LOG::trace('ENGINE_MCRYPT engine used for SFTP encryption');
            if ($this->changed) {
                $this->_setupMcrypt();
                $this->changed = false;
            }
        ...
        \DUP_PRO_LOG::trace('INTERNAL engine used for SFTP encryption');
        if ($this->changed) {
            $this->_setup();
            $this->changed = false;
        }
        ...
        switch ($engine) {
            ...
            case self::ENGINE_MCRYPT:
                return $this->cipher_name_mcrypt &&
                        // The below condition added by the SnapCreek. mcrypt_generic() func deprected from PHP 7.1 has been DEPRECATED as of PHP 7.1.0 and REMOVED as of PHP 7.2.0.
