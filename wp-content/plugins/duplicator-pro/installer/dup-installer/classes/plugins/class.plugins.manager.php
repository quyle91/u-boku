...
final class DUPX_Plugins_Manager
{
    ...
    private function setCustomPluginsActions()
    {
        ...
        $this->customPluginsActions[self::SLUG_WPS_HIDE_LOGIN]  = new DUPX_Plugin_custom_actions(self::SLUG_WPS_HIDE_LOGIN, DUPX_Plugin_custom_actions::BY_DEFAULT_DISABLED, true, $longMsg);
    ...
    {
        $result         = array();
        $networkInstall = DUPX_MU::newSiteIsMultisite();
        foreach ($this->plugins as $plugin) {
            if ($networkInstall) {
    ...
    public function executeActions($dbh, $subsiteId = -1)
    {
        ...
        foreach ($this->plugins as $plugin) {
            ...
            if ($plugin->deactivateAction) {
                ...
                $deactivated = true;
            } elseif (DUPX_MU::newSiteIsMultisite()) {
        ...
        if (DUPX_MU::newSiteIsMultisite()) {
            ...
            if (DUPX_InstallerState::isAddSiteOnMultisite()) {
                $optionTable = mysqli_real_escape_string($dbh, DUPX_DB_Functions::getOptionsTableName(DUPX_MU::getSubsiteOverwriteTablePrefix()));
            } else {
