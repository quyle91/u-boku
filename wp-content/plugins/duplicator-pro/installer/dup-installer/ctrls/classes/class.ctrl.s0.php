...
final class DUPX_Ctrl_S0
{
    ...
    public static function stepHeaderLog()
    {
        ...
        if (DUPX_InstallerState::isImportFromBackendMode() || DUPX_InstallerState::isRecoveryMode()) {
            ...
            Log::info(str_pad('LICENSE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . License::getLicenseToString(License::getImporterLicense()));
            Log::info("********************************************************************************");
        }
        ...
        $log .= str_pad('LICENSE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . License::getLicenseToString(License::getInstallerLicense()) . "\n";
